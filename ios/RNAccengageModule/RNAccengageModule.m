...
- (void)getAccengageMessagesWithSuccess:(void (^)(NSMutableArray *messages))success failure:(RCTPromiseRejectBlock)failure
{
    ...
    if(_numLoadedMessages < _inboxMessageList.size)
    {
        int unreadMessagesCount = (int)(_inboxMessageList.size - _numLoadedMessages);
        _numLoadedMessages = MIN(unreadMessagesCount, 10);
        
        for (int i = 0; i < _numLoadedMessages; i++)
        {
